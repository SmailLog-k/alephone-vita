...
static Uint64 vita_perf_update_screen_total;
static Uint64 vita_perf_draw_surface_total;
static Uint64 vita_perf_darken_total;
static Uint64 vita_perf_present_total;
static Uint64 vita_perf_frame_total;
static uint32 vita_perf_frames;
static uint32 vita_perf_present_calls;
static int vita_perf_world_w;
static int vita_perf_world_h;
static int vita_perf_world_bpp;
static int vita_perf_main_w;
static int vita_perf_main_h;
static int vita_perf_main_bpp;

static inline Uint64 vita_perf_now_us()
{
	return sceKernelGetProcessTimeWide();
}

static void vita_perf_record_present(Uint64 elapsed_us)
{
	vita_perf_present_total += elapsed_us;
}

static void vita_perf_maybe_log()
{
	...
	if (file) {
		fprintf(file,
			...
			(unsigned)vita_perf_frames,
			(unsigned long long)(vita_perf_frame_total / vita_perf_frames),
			(unsigned long long)(vita_perf_render_view_total / vita_perf_frames),
			(unsigned long long)(vita_perf_overlay_total / vita_perf_frames),
			(unsigned long long)(vita_perf_blit_total / vita_perf_frames),
			(unsigned long long)(vita_perf_update_screen_total / vita_perf_frames),
			(unsigned long long)(vita_perf_draw_surface_total / vita_perf_frames),
			(unsigned long long)(vita_perf_darken_total / vita_perf_frames),
			(unsigned long long)(vita_perf_present_total / vita_perf_frames),
			(unsigned)(vita_perf_present_calls / vita_perf_frames),
			vita_perf_world_w,
			vita_perf_world_h,
			vita_perf_world_bpp,
			vita_perf_main_w,
	...
	vita_perf_render_view_total = 0;
	vita_perf_overlay_total = 0;
	vita_perf_blit_total = 0;
	vita_perf_update_screen_total = 0;
	vita_perf_draw_surface_total = 0;
	vita_perf_darken_total = 0;
	vita_perf_present_total = 0;
	vita_perf_frame_total = 0;
	vita_perf_frames = 0;
	vita_perf_present_calls = 0;
}
#endif
...
static void vita_draw_minimal_hud(SDL_Surface *s)
{
	...
	const int gap = 8;
	const int x = margin;
	const int y = s->h - margin - (bar_h * 2 + gap);

	const auto draw_bar = [&](int by, int value, int maximum, uint8 r, uint8 g, uint8 b) {
		value = std::max(0, std::min(value, maximum));
		SDL_Rect background = { x, by, bar_w, bar_h };
		SDL_Rect border = { x - 1, by - 1, bar_w + 2, bar_h + 2 };
		SDL_Rect filled = { x, by, maximum > 0 ? (bar_w * value) / maximum : 0, bar_h };
		SDL_FillRect(s, &border, SDL_MapRGB(s->format, 0xff, 0xff, 0xff));
		SDL_FillRect(s, &background, SDL_MapRGB(s->format, 0x08, 0x08, 0x08));
		SDL_FillRect(s, &filled, SDL_MapRGB(s->format, r, g, b));
	};

	const int shield_layer_value = current_player->suit_energy % PLAYER_MAXIMUM_SUIT_ENERGY;
	const int shield_value = std::max(0, std::min<int>(
		(shield_layer_value == 0 && current_player->suit_energy > 0) ? PLAYER_MAXIMUM_SUIT_ENERGY : shield_layer_value,
		PLAYER_MAXIMUM_SUIT_ENERGY));
	const int oxygen_value = std::max(0, std::min<int>(current_player->suit_oxygen, PLAYER_MAXIMUM_SUIT_OXYGEN));

	draw_bar(y, shield_value, PLAYER_MAXIMUM_SUIT_ENERGY, 0x20, 0xe0, 0x50);
	draw_bar(y + bar_h + gap, oxygen_value, PLAYER_MAXIMUM_SUIT_OXYGEN, 0x30, 0x90, 0xff);
	...
	char text[64];
	snprintf(text, sizeof(text), "SHIELD %d%%", (std::max<int>(0, current_player->suit_energy) * 100) / PLAYER_MAXIMUM_SUIT_ENERGY);
	DisplayText(x + bar_w + 8, y + bar_h, text, 0x90, 0xff, 0x90);
	snprintf(text, sizeof(text), "O2 %d%%", (oxygen_value * 100) / PLAYER_MAXIMUM_SUIT_OXYGEN);
	DisplayText(x + bar_w + 8, y + bar_h * 2 + gap, text, 0x90, 0xc0, 0xff);

	const int weapon = get_player_desired_weapon(current_player_index);
	const int right_x = s->w - 210;
	const int right_y = y;
	if (weapon >= 0 && weapon < MAXIMUM_NUMBER_OF_WEAPONS) {
		const int primary = get_player_weapon_ammo_count(current_player_index, weapon, _primary_weapon);
		const int primary_max = get_player_weapon_ammo_maximum(current_player_index, weapon, _primary_weapon);
		const int primary_type = get_player_weapon_ammo_type(current_player_index, weapon, _primary_weapon);
		const int primary_reserve = (primary_type >= 0 && primary_type < NUMBER_OF_ITEMS && current_player->items[primary_type] > 0)
			? current_player->items[primary_type] : 0;
		const int secondary = get_player_weapon_ammo_count(current_player_index, weapon, _secondary_weapon);
		const int secondary_max = get_player_weapon_ammo_maximum(current_player_index, weapon, _secondary_weapon);
		const int secondary_type = get_player_weapon_ammo_type(current_player_index, weapon, _secondary_weapon);
		const int secondary_reserve = (secondary_type >= 0 && secondary_type < NUMBER_OF_ITEMS && current_player->items[secondary_type] > 0)
			? current_player->items[secondary_type] : 0;

		char weapon_name[64];
#define strWEAPON_NAME_LIST 137
		getcstr(weapon_name, strWEAPON_NAME_LIST, weapon);
#undef strWEAPON_NAME_LIST
		if (weapon_name[0] == '\0')
			snprintf(weapon_name, sizeof(weapon_name), "WEAPON %d", weapon + 1);
		weapon_name[sizeof(weapon_name) - 1] = '\0';

		snprintf(text, sizeof(text), "%s", weapon_name);
		DisplayText(right_x, right_y + bar_h, text, 0xff, 0xe0, 0x80);
		if (primary >= 0 && primary_max > 0) {
			snprintf(text, sizeof(text), "PRI %d/%d +%d", primary, primary_max, primary_reserve);
			DisplayText(right_x, right_y + bar_h * 2 + gap, text, 0xff, 0xff, 0xff);
		}
		if (secondary >= 0 && secondary_max > 0) {
			snprintf(text, sizeof(text), "SEC %d/%d +%d", secondary, secondary_max, secondary_reserve);
			DisplayText(right_x, right_y + bar_h * 3 + gap * 2, text, 0xff, 0xff, 0xff);
		}
	} else {
		DisplayText(right_x, right_y + bar_h, "WEAPON --", 0xff, 0xe0, 0x80);
	}
}
#endif
...
{
#ifdef VITA
	Uint64 vita_frame_start_us = vita_perf_now_us();
	Uint64 vita_section_start_us = 0;
#endif
	...
	render_view(world_view, software_render_dest.get());
#ifdef VITA
	vita_perf_render_view_total += vita_perf_now_us() - vita_section_start_us;
#endif
...
#ifdef VITA
		vita_perf_blit_total += vita_perf_now_us() - vita_section_start_us;
#endif
	}
...
{
#ifdef VITA
	Uint64 vita_update_screen_start_us = vita_perf_now_us();
#endif
...
#ifdef VITA
	vita_perf_world_w = s ? s->w : 0;
	vita_perf_world_h = s ? s->h : 0;
	vita_perf_world_bpp = s ? s->format->BitsPerPixel : 0;
	vita_perf_main_w = main_surface ? main_surface->w : 0;
	vita_perf_main_h = main_surface ? main_surface->h : 0;
	vita_perf_main_bpp = main_surface ? main_surface->format->BitsPerPixel : 0;
	vita_perf_update_screen_total += vita_perf_now_us() - vita_update_screen_start_us;
#endif
...
{
#ifdef VITA
	Uint64 vita_darken_start_us = vita_perf_now_us();
	vita_perf_darken_total += vita_perf_now_us() - vita_darken_start_us;
	return;
#endif
...
{
#ifdef VITA
	Uint64 vita_draw_surface_start_us = vita_perf_now_us();
#endif
	if (s) {
...
#ifdef VITA
	vita_perf_draw_surface_total += vita_perf_now_us() - vita_draw_surface_start_us;
#endif
}
...
{
#ifdef VITA
	Uint64 vita_present_start_us = vita_perf_now_us();
	vita_perf_present_calls++;
#endif
	bool update_full_texture = count == 0 || rects == NULL;
	...
	if (!update_full_texture) {
		for (size_t i = 0; i < count; ++i) {
			...
			if (!SDL_IntersectRect(&rects[i], &surface_bounds, &clipped_rect)) {
				continue;
			}

			const uint8 *pixels = (const uint8 *)main_surface->pixels
				+ clipped_rect.y * main_surface->pitch
				+ clipped_rect.x * main_surface->format->BytesPerPixel;
			SDL_UpdateTexture(main_texture, &clipped_rect, pixels, main_surface->pitch);
			updated_any_rect = true;
		}
	}

	if (update_full_texture || !updated_any_rect)
#endif
	{
...
#ifdef VITA
	vita_perf_record_present(vita_perf_now_us() - vita_present_start_us);
#endif
}
